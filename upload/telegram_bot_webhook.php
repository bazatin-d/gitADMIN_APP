...
function asr_tg_webhook_runtime_fallback(PDO $pdo, int $botId, string $rawBody): void {
    ...
    try {
        ...
        try {
            asr_tg_log($pdo, $botId, 'info', 'webhook_wrapper_runtime_start', 'Wrapper запускает сценарий по команде.', ['command' => $command, 'scenario_id' => $scenarioId, 'step_id' => $stepId, 'subscriber_id' => $subscriberId, 'command_id' => (int)($row['id'] ?? 0)]);
        } catch (Throwable $ignored) {}
