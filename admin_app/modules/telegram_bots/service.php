    ...
    if ($subscriberId > 0) {
        try {
            ...
            if ((string)$blockedCheck->fetchColumn() === 'blocked') {
                asr_tg_log($pdo, $botId, 'info', 'webhook_blocked_subscriber_ignored', 'Сообщение от заблокированного подписчика не добавлено в диалоги.', ['subscriber_id' => $subscriberId]);
                return;
            }
        } catch (Throwable $e) {}
    }
