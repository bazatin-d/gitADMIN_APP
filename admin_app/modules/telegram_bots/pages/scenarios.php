...
try {
    ...
    $scenarioBotMap = [];
    $scenarioDefaultBotMap = [];
    foreach ($scenarios as $row) {
        $sid = (int)($row['id'] ?? 0);
        if ($sid > 0) {
            $scenarioBotMap[$sid] = asr_tg_scenario_bot_ids($pdo, $sid, false);
            $scenarioDefaultBotMap[$sid] = asr_tg_scenario_default_bot_id($pdo, $sid);
        }
    }
} catch (Throwable $e) { ?>
    ...
    <?php else: ?>
        <table class="tg-scenario-table">
            ...
            <?php foreach ($scenarios as $row):
                ...
                $botIds = $scenarioBotMap[$rowId] ?? [];
                $defaultBotId = (int)($scenarioDefaultBotMap[$rowId] ?? 0);
                $rowData = [
                    'id' => $rowId,
                ...
                <tr>
                    ...
                    <td><div class="tg-scenario-channels"><?php echo $channels !== '' ? $h($channels) : 'Каналы пока не подключены'; ?></div></td>
...
<div class="tg-scenario-modal" data-scenario-modal>
    <div class="tg-scenario-modal-head">
        <div>
            ...
            <div class="tg-scenarios-note">Базовые настройки сценария: название, описание, статус и подключённые каналы.</div>
        </div>
    ...
    <form method="POST" class="tg-scenario-form" data-scenario-form>
        ...
        <div class="tg-scenario-field">
            <span>Каналы сценария</span>
            <div class="tg-scenario-bots">
                <?php if (!$botsLight): ?>
                ...
                <?php foreach ($botsLight as $bot):
                    ...
                    ?>
                    <label class="tg-scenario-bot-row">
                        <input type="checkbox" name="scenario_bot_ids[]" value="<?php echo $botId; ?>" data-scenario-bot-checkbox>
                        <span>
                            <span class="tg-scenario-bot-title"><?php echo $h($bot['title'] ?? ('Канал #' . $botId)); ?></span>
                            <span class="tg-scenario-bot-meta"><?php echo $h($channelType); ?><?php echo !empty($bot['bot_username']) ? ' · @' . $h($bot['bot_username']) : ''; ?></span>
                        </span>
                        <span class="tg-scenario-default"><input type="radio" name="default_bot_id" value="<?php echo $botId; ?>" data-scenario-default-radio> основной</span>
                    </label>
                <?php endforeach; ?>
            </div>
        </div>

        <div class="tg-scenario-help">Активный сценарий должен иметь хотя бы один канал. Если сценарий подключён к нескольким каналам, конкретное прохождение подписчика будет идти через тот канал, где оно стартовало.</div>
...
(function(){
    ...
    const botChecks = Array.from(form.querySelectorAll('[data-scenario-bot-checkbox]'));
    const defaultRadios = Array.from(form.querySelectorAll('[data-scenario-default-radio]'));
    ...
    function resetBots(){
        botChecks.forEach(cb => cb.checked = false);
    ...
    function openEdit(data){
        ...
        resetBots();
        const ids = Array.isArray(data.bot_ids) ? data.bot_ids.map(Number) : [];
        const def = Number(data.default_bot_id || 0);
        botChecks.forEach(cb => cb.checked = ids.includes(Number(cb.value)));
        defaultRadios.forEach(rb => rb.checked = Number(rb.value) === def);
        if (!defaultRadios.some(rb => rb.checked)) {
            const firstChecked = botChecks.find(cb => cb.checked);
            if (firstChecked) {
                const radio = defaultRadios.find(rb => rb.value === firstChecked.value);
    ...
    botChecks.forEach(cb => {
        cb.addEventListener('change', function(){
            const radio = defaultRadios.find(rb => rb.value === this.value);
            if (this.checked && radio && !defaultRadios.some(rb => rb.checked)) radio.checked = true;
            if (!this.checked && radio && radio.checked) {
                radio.checked = false;
                const firstChecked = botChecks.find(item => item.checked);
                if (firstChecked) {
                    const nextRadio = defaultRadios.find(rb => rb.value === firstChecked.value);
                    if (nextRadio) nextRadio.checked = true;
                }
            }
        });
    });
    defaultRadios.forEach(rb => {
        rb.addEventListener('change', function(){
            const cb = botChecks.find(item => item.value === this.value);
