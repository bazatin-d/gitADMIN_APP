...
@media(max-width:768px){
  ...
  .tb2-title-icon{width:24px!important;height:24px!important;flex-basis:24px!important}
                ...
                <div class="mt-6 rounded-3xl border border-orange-100 bg-orange-50/40 p-5">
                    ...
                    <div class="mt-5 rounded-2xl bg-white border border-gray-100 p-4">
                        ...
                        <?php if ($reportCards): foreach ($reportCards as $idx => $card): ?>
                            <div class="mt-3 rounded-2xl bg-gray-50 border border-gray-100 p-4"><div class="text-xs font-semibold text-gray-500">Карточка <?php echo (int)$idx + 1; ?> · <?php echo $h($card['type'] ?? 'text'); ?></div><div class="mt-3 tg-report-phone"><div class="tg-report-bubble"><?php if (!empty($card['media_url'])): ?><div class="tg-report-media">Медиа: <a class="text-orange-600 underline" target="_blank" rel="noopener" href="<?php echo $h($card['media_url']); ?>"><?php echo $h($card['media_file_name'] ?? $card['media_url']); ?></a></div><?php endif; ?><?php echo $asrTgRenderReportText((string)($card['text'] ?? '')); ?><?php echo $asrTgRenderReportButtons(is_array($card) ? $card : []); ?></div></div></div>
                        <?php endforeach; else: ?>
                    ...
                    <div class="mt-6">
                        ...
                        <div class="tg-channel-picker" id="tgBotPicker">
                            ...
                            <div class="tg-channel-picker-menu" id="tgBotDropdownPanel" role="listbox" aria-label="Каналы для рассылки">
                                ...
                                <?php foreach ($activeBroadcastBots as $bot): ?>
                                    <?php $botLabel = $bot['title'] . (($bot['bot_username'] ?? '') ? ' (@' . $bot['bot_username'] . ')' : ''); ?>
                                    <label class="tg-channel-picker-item">
                                        <input type="checkbox" data-bot-option value="<?php echo (int)$bot['id']; ?>">
                                        <span><?php echo $h($botLabel); ?></span>
                                        <span class="tg-channel-picker-count"><?php echo (int)($bot['subscribers_count'] ?? 0); ?></span>
                                    </label>
                                <?php endforeach; ?>
                        ...
                        <select name="bot_ids[]" id="tgBotMultiSelect" multiple class="tg-hidden-select" aria-hidden="true" tabindex="-1">
                            <?php foreach ($activeBroadcastBots as $bot): ?>
                                <?php $botLabel = $bot['title'] . (($bot['bot_username'] ?? '') ? ' (@' . $bot['bot_username'] . ')' : ''); ?>
                                <option value="<?php echo (int)$bot['id']; ?>" data-label="<?php echo $h($botLabel); ?>" data-count="<?php echo (int)($bot['subscribers_count'] ?? 0); ?>" selected><?php echo $h($botLabel); ?> · <?php echo (int)($bot['subscribers_count'] ?? 0); ?></option>
                            <?php endforeach; ?>
                        </select>
                        <label class="mt-5 ml-1 flex items-start gap-2 text-xs font-semibold text-gray-500 cursor-pointer max-w-xl">
                            ...
                            <span><span class="block text-gray-600">Отправить дубли, если человек подписан на несколько каналов</span><span class="block mt-1 text-[11px] text-gray-400">По умолчанию один Telegram-пользователь получит сообщение только один раз.</span></span>
                        </label>
                    </div>
    ...
    <div class="tg-button-modal-backdrop" id="tgButtonModal" aria-hidden="true">
        <div class="tg-button-modal">
            ...
            <div class="tg-button-modal-body space-y-4">
                ...
                <label class="block"><span class="tg-form-label">Тип кнопки</span><select class="tg-button-select" id="tgButtonType"><option value="url">URL</option><option value="action">Действие</option></select></label>
                ...
                <div id="tgButtonActionWrap" class="hidden rounded-xl bg-gray-50 border border-gray-100 p-4 text-sm font-semibold text-gray-500">Действия подключим на следующем этапе сценариев. Сейчас этот тип можно выбрать только как заготовку.</div>
            </div>
    ...
    (function(){
        ...
        const next=form.querySelector('[data-step-next]'), prev=form.querySelector('[data-step-prev]'), testSendBtn=document.getElementById('tgBroadcastTestSend'), testResult=document.getElementById('tgBroadcastTestResult'), cardsBox=document.getElementById('tgCardsBox'), preview=document.getElementById('tgPreview');
        ...
        function setBotSelected(id, selected){const opt=botOptions().find(o=>o.value===String(id)); if(opt) opt.selected=!!selected; const cb=botCheckboxes().find(c=>c.value===String(id)); if(cb) cb.checked=!!selected;}
        function syncBotCheckboxesFromSelect(){const ids=new Set(selectedBotIds()); botCheckboxes().forEach(cb=>{cb.checked=ids.has(cb.value);});}
        function updateBotPickerSummary(){if(!botPickerSummary)return; const opts=botOptions(); const selected=selectedBotOptions(); if(opts.length>0 && selected.length===opts.length){botPickerSummary.textContent='Все каналы'; return;} if(selected.length===0){botPickerSummary.textContent='Выберите каналы'; return;} if(selected.length===1){botPickerSummary.textContent=selected[0].dataset.label||selected[0].textContent.replace(/·.*$/,'').trim(); return;} botPickerSummary.textContent='Выбрано каналов: '+selected.length;}
        function syncSelectedBots(){syncBotCheckboxesFromSelect(); const ids=selectedBotIds(); if(botSelect) botSelect.value=ids[0]||''; if(allBotsToggle){const opts=botOptions(); allBotsToggle.checked=opts.length>0 && opts.every(opt=>opt.selected); allBotsToggle.indeterminate=ids.length>0 && ids.length<opts.length;} updateBotPickerSummary();}
...
function adminIcon(name){
    ...
    return '<img class="tg-ui-icon tg-ui-icon--toolbar" src="/assets/admin/icons/tg2-'+file+'-gray.svg?v=20260530-tg2-gray" alt="" aria-hidden="true">';
        ...
        btnSave?.addEventListener('click',()=>{if(!buttonContext)return; const text=(btnText.value||'').trim(); if(!text){showAlert('Введите название кнопки.');return;} const type=btnType.value==='action'?'action':'url'; const url=(btnUrl.value||'').trim(); if(type==='url'){if(!url){showAlert('Укажите ссылку кнопки.');return;} const urlError=validatePublicHttpUrl(url,'Ссылка кнопки'); if(urlError){showAlert(urlError);return;}} const rows=readButtons(buttonContext.card); let ri=buttonContext.rowIndex; if(ri===null || ri===undefined){ri=rows.length; rows[ri]=[];} if(!rows[ri]) rows[ri]=[]; const item={text:text,type:type,url:type==='url'?url:'',color:'primary'}; if(buttonContext.isNew){rows[ri].splice(buttonContext.buttonIndex||0,0,item);} else {rows[ri][buttonContext.buttonIndex]=item;} writeButtons(buttonContext.card, rows.filter(r=>r.length)); buttonContext.card.classList.remove('is-invalid'); buttonContext.card.removeAttribute('data-card-error'); closeButtonModal();});
        ...
        function mediaPreviewHtml(card,type){if(type==='text')return''; const file=card.querySelector('[data-media-file]')?.files?.[0]; const url=(card.querySelector('[data-media-url]')?.value||'').trim(); if(file && file.type && file.type.startsWith('image/')){if(card.dataset.mediaPreviewUrl) URL.revokeObjectURL(card.dataset.mediaPreviewUrl); card.dataset.mediaPreviewUrl=URL.createObjectURL(file); return `<div class="tg-card-preview-media"><img src="${card.dataset.mediaPreviewUrl}" alt="Картинка"><div>${esc(file.name)}</div></div>`;} if(url && /\.(png|jpe?g|gif|webp)(\?|#|$)/i.test(url)){return `<div class="tg-card-preview-media"><img src="${esc(url)}" alt="Картинка"><div>Картинка</div></div>`;} const label=file?.name || (url?url:cardTitle(type)); return `<div class="tg-card-preview-media">${esc(label)}</div>`;}
        function updatePreview(){let html='<div class="text-xs font-semibold text-gray-500 mb-3">Предпросмотр</div>';cardsBox.querySelectorAll('[data-card]').forEach(card=>{const type=card.querySelector('input[name="card_type[]"]').value; const text=syncCard(card); html+='<div class="tg-preview-bubble mb-3">'; html+=mediaPreviewHtml(card,type); html+=text?htmlPreview(text):'<span class="text-gray-400">Пустая карточка</span>'; const rows=readButtons(card); if(rows.length){html+='<div class="tg-preview-buttons">'; rows.forEach(row=>{html+='<div class="tg-preview-button-row">'; row.forEach(btn=>{html+=`<div class="tg-preview-button">${esc(btn.text||'Кнопка')}</div>`;}); html+='</div>';}); html+='</div>'; } html+='</div>';});preview.innerHTML=html; saveDraft();}
        ...
        async function sendTestBroadcast(){
            ...
            fd.set('tg_ajax','1');
            if(testSendBtn){testSendBtn.disabled=true; testSendBtn.textContent='Отправляю тест...';}
            if(testResult){testResult.textContent='Отправляю тестовую рассылку в технический бот.'; testResult.className='mt-3 text-xs font-semibold text-gray-500';}
            try{
                ...
                if(testSendBtn){testSendBtn.disabled=false; testSendBtn.innerHTML='<img class="tb2-icon" src="/assets/admin/icons/sh2-send-gray.svg" alt="" aria-hidden="true">Отправить тест в тех. бот';}
            }
        }
        ...
        function setAllBotOptions(checked){botOptions().forEach(opt=>{opt.selected=!!checked;}); commitBotPickerChange();}
        function toggleAllBotOptions(){const opts=botOptions(); if(!opts.length)return; const next=!opts.every(opt=>opt.selected); setAllBotOptions(next);}
        function toggleOneBotOption(cb){if(!cb)return; const opt=botOptions().find(o=>o.value===String(cb.value)); const next=!(opt&&opt.selected); setBotSelected(cb.value,next); commitBotPickerChange();}
