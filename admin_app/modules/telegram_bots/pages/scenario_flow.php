...
$getActionSettings = static function(array $block): array {
    $settings = json_decode((string)($block['settings_json'] ?? ''), true);
    if (!is_array($settings)) $settings = [];
    ...
    foreach ($actions as $item) {
        ...
        $summary = trim((string)($item['summary'] ?? $item['title'] ?? 'Действие'));
        if ($summary !== '') $summaries[] = $summary;
        if (count($summaries) >= 4) break;
    }
    $count = count($actions);
    return [
        'actions' => $actions,
        'actionsCount' => $count,
        'summaries' => array_slice($summaries, 0, 3),
        'extraCount' => max(0, $count - 3),
        'isInvalid' => $count <= 0 || empty($settings['actions_valid']),
        'preview' => $count > 0 ? implode('; ', array_slice($summaries, 0, 3)) : 'Не добавлено ни одного действия',
    ];
};
...
$scenarioStats = asr_tg_scenario_stats_for_scenario($pdo, $scenarioId);
$nodes = [];
foreach ($blocks as $index => $block) {
    ...
    $preview = $isStart ? ($currentChannelType === 'vk' ? 'По входящему сообщению VK / кнопке' : 'По кнопке «Начать»') : ($isDelay ? (string)($delaySettings['preview'] ?? 'Подождать 5 минут') : ($isCondition ? (string)($conditionSettings['preview'] ?? 'Условие не настроено') : ($isActions ? (string)($actionSettings['preview'] ?? 'Не добавлено ни одного действия') : ($isSchedule ? (string)($scheduleSettings['preview'] ?? 'Расписание не настроено') : ($isRandom ? (string)($randomSettings['preview'] ?? 'Случайный выбор') : (string)($firstCard['textPreview'] ?? '')))))); 
    $deeplink = (!$isStart && $currentChannelType === 'telegram') ? ($deeplinksByBlock[$blockId] ?? null) : null;
    ...
    $testLink = (!$isStart && $currentChannelType === 'telegram' && function_exists('asr_tg_scenario_test_link_for_block')) ? asr_tg_scenario_test_link_for_block($pdo, $scenarioId, $blockId) : ['enabled' => false, 'url' => '', 'reason' => ($currentChannelType === 'vk' ? 'Тест с этого шага через Telegram-ссылку недоступен для VK-сценария.' : '')];
    ...
    $nodes[] = [
        ...
        'data' => [
            ...
            'actionsInvalid' => !empty($actionSettings['isInvalid']),
            'actionsCount' => (int)($actionSettings['actionsCount'] ?? 0),
            'actionsExtraCount' => (int)($actionSettings['extraCount'] ?? 0),
            'actionsSummaries' => $actionSettings['summaries'] ?? [],
            'cards' => $flowCards,
...
$currentScenarioVkScreenName = is_array($currentScenarioBot) ? trim((string)($currentScenarioBot['vk_screen_name'] ?? '')) : '';
$currentScenarioVkUrl = ($currentChannelType === 'vk' && $currentScenarioVkScreenName !== '') ? ('https://vk.com/' . rawurlencode(ltrim($currentScenarioVkScreenName, '@'))) : '';
...
// она используется только как транспорт для Telegram-диплинков сценариев.
// Для VK меню команд и Telegram-диплинки не показываем: VK запускается входящими сообщениями и payload-кнопками.
...
$flowData = [
    ...
    'channelLabel' => $currentChannelType === 'vk' ? 'VK' : 'Telegram',
    'csrfToken' => $csrf,
    'nodes' => $nodes,
    'edges' => $edges,
...
.tg-flow-block-drawer .tg-message-button{min-height:38px;flex:1;border:1px solid #dbeafe;border-radius:8px;background:#fff;color:#FFA048;font-size:13px;font-weight:600;position:relative;cursor:pointer;}
.tg-flow-block-drawer .tg-message-button:hover{background:#f8fbff;}
.tg-flow-block-drawer .tg-message-button-add{width:38px;height:38px;border:1px solid #dbeafe;border-radius:8px;background:#fff;color:#FFA048;font-size:20px;line-height:1;cursor:pointer;}
...
.tg-flow-block-drawer .tg-card-editor a{color:#FFA048;text-decoration:underline;}
...
.tg-flow-panel-menu-wrap{position:relative;display:inline-flex;align-items:center}.tg-flow-panel-more{width:44px;height:44px;border:1px solid #edf0f2;background:#fff;border-radius:16px;color:#6b7280;font-size:22px;font-weight:700;display:inline-flex;align-items:center;justify-content:center;cursor:pointer}.tg-flow-panel-more:hover{background:#fff7ed;border-color:#fed7aa;color:#9a5a1f}.tg-flow-panel-dropdown{position:absolute;right:0;top:52px;z-index:80;width:250px;background:#fff;border:1px solid #e5e7eb;border-radius:18px;box-shadow:0 20px 50px rgba(15,23,42,.18);padding:7px;display:none}.tg-flow-panel-dropdown.is-open{display:block}.tg-flow-panel-dropdown button{width:100%;height:auto;border:0;background:#fff;border-radius:13px;padding:12px 13px;display:flex;align-items:center;justify-content:flex-start;gap:11px;text-align:left;font-size:14px;font-weight:500;color:#4b5563;cursor:pointer}.tg-flow-panel-dropdown button:hover{background:#fff7ed;color:#9a5a1f}.tg-flow-panel-dropdown button[disabled]{cursor:not-allowed;background:#fff;color:#c2c7d0}.tg-flow-panel-dropdown button.is-danger:hover{background:#fef2f2;color:#dc2626}.tg-flow-panel-actions{display:flex;align-items:center;gap:10px}.tg-flow-panel-menu-note{display:block;font-size:11px;color:#a8afb9;font-weight:500;margin-top:2px;text-align:left}.tg-flow-panel-menu-main{display:block;text-align:left;font-weight:500}.tg-flow-panel-dropdown-ico{width:20px;text-align:center;color:inherit;flex:0 0 20px}.tg-flow-panel-block-id{display:block;margin-top:6px;color:#9ca3af;font-size:12px;font-weight:500;line-height:1.35}

.react-flow__edge.is-selected .react-flow__edge-path,.react-flow__edge.selected .react-flow__edge-path{stroke:#2f80ed!important;stroke-width:3!important}.tg-flow-edge-delete-hint{position:absolute;right:18px;bottom:54px;z-index:5;background:#fff;border:1px solid #dbeafe;color:#FFA048;border-radius:14px;padding:8px 12px;font-size:12px;font-weight:700;box-shadow:0 10px 24px rgba(15,23,42,.08)}
...
/* v3.5.144: actions block visual state */
.tg-flow-add-icon--actions{background:#f2eaff!important;color:#7c5ac8!important;}
.tg-flow-node.is-actions{border-color:#e4d7ff;min-width:300px}
.tg-flow-node.is-actions .tg-flow-node-head{background:#f2eaff!important;border-bottom-color:#e4d7ff!important}
.tg-flow-node.is-actions .tg-flow-node-title{color:#6f54bd!important}
.tg-flow-node.is-actions.is-actions-invalid{border-color:#ff6b73!important;box-shadow:0 0 0 1px #ff6b73,0 14px 28px rgba(255,107,115,.14)!important}
.tg-flow-node.is-actions.is-actions-invalid .tg-flow-main-out-handle{border-color:#ff6b73!important;box-shadow:0 0 0 3px #fff,0 0 0 5px rgba(255,107,115,.24)!important}
.tg-flow-node-card.tg-flow-actions-preview{background:#fff;color:#374151;border:0;display:flex;flex-direction:column;gap:8px;min-height:86px;padding:10px}
.tg-flow-actions-preview-head{display:flex;align-items:center;justify-content:space-between;gap:8px;font-size:11px;font-weight:750;color:#6b7280;line-height:1.3}
.tg-flow-actions-preview-count{display:inline-flex;align-items:center;justify-content:center;min-width:22px;height:20px;border-radius:999px;background:#f2eaff;color:#6f54bd;font-size:11px;font-weight:800}
.tg-flow-actions-list{display:flex;flex-direction:column;gap:6px}
.tg-flow-actions-row{border:1px solid #edf0f2;background:#f8fafc;border-radius:9px;padding:7px 8px;color:#4b5563;font-size:11px;font-weight:650;line-height:1.35;word-break:break-word}
.tg-flow-actions-more{color:#8b7bb4;font-size:11px;font-weight:700;padding:0 2px}
.tg-flow-actions-empty{border:1px dashed #fecaca;background:#fff7f7;border-radius:9px;padding:9px;color:#ef4444;font-size:11px;font-weight:700;line-height:1.35}
...
<div class="tg-flow-app">
    <div class="tg-flow-topbar">
        <div class="tg-flow-top-left">
            ...
            <div class="tg-flow-title"><?php echo $h($scenario['title'] ?? 'Сценарий'); ?> <span class="tg-flow-platform-badge <?php echo $currentChannelType === 'vk' ? 'is-vk' : 'is-tg'; ?>"><?php echo $currentChannelType === 'vk' ? 'VK-сценарий' : 'Telegram-сценарий'; ?></span> <span style="font-size:11px;color:#9ca3af;font-weight:650">Flow v3.6.34</span> <span id="tg-flow-boot-status" class="tg-flow-boot-status">PHP: <?php echo count($nodes); ?> блоков / <?php echo count($edges); ?> связей · React: запуск…</span></div>
        </div>
        <div class="tg-flow-top-right">
            <div class="tg-flow-gear-wrap">
                ...
                <div class="tg-flow-gear-menu" id="tg-flow-gear-menu">
                    <?php if ($currentChannelType === 'telegram' && $currentScenarioBotUrl !== ''): ?>
                    ...
                    <?php elseif ($currentChannelType === 'vk' && $currentScenarioVkUrl !== ''): ?>
                        <a class="tg-flow-gear-link" href="<?php echo $h($currentScenarioVkUrl); ?>" target="_blank" rel="noopener"><span class="tg-flow-gear-ico">↗</span><span>Открыть сообщество VK</span></a>
                    <?php endif; ?>
        ...
        <form method="POST">
            ...
            <div class="tg-flow-modal-body">
                ...
                <label class="tg-flow-field"><span>Канал сценария</span><select name="scenario_bot_id">
                    ...
                    <?php endforeach; ?>
                </select><small class="tg-flow-platform-note-inline"><?php echo $currentChannelType === 'vk' ? 'Сейчас включён VK-режим: Telegram-меню, диплинки и Telegram-форматирование скрываются. При смене канала редактор подстроится после сохранения.' : 'Сейчас включён Telegram-режим: доступны Telegram-меню, диплинки, форматирование и Telegram-медиа.'; ?></small></label>
            </div>
