...
$safeHumanizeColumn = static function(string $code): string {
    $labels = [
        ...
        'chat_id' => 'Chat ID',
        'external_user_id' => 'Внешний User ID',
        'external_chat_id' => 'Внешний Chat ID',
        'username' => 'Username',
        'first_name' => 'Имя',
        'last_name' => 'Фамилия',
        'phone' => 'Телефон',
...
try {
    $safeBots = [];
    $safeBotStmt = $pdo->query("SELECT id,title,bot_username,channel_type,vk_screen_name,status FROM oca_telegram_bots ORDER BY id ASC LIMIT 200");
    ...
    try {
        ...
        $safeAllowedColumns = [
            'id'=>'int','bot_id'=>'int','platform'=>'varchar','telegram_user_id'=>'bigint','chat_id'=>'bigint','external_user_id'=>'varchar','external_chat_id'=>'varchar','username'=>'varchar',
    ...
    $sql = "SELECT s.id,s.bot_id,s.platform,s.telegram_user_id,s.chat_id,s.external_user_id,s.external_chat_id,s.username,s.first_name,s.last_name,s.status,s.first_seen_at,s.last_seen_at,b.title AS bot_title,b.bot_username,b.channel_type,b.vk_screen_name
...
<section class="tg-subs-emergency">
        ...
        <div class="tg-filter-mainline">
            <select name="bot_id" class="tg-filter-channel" data-tg-filter-channel>
                ...
                <?php foreach ($safeBots as $bot): ?>
                    <?php $botType = strtolower((string)($bot['channel_type'] ?? 'telegram')) === 'vk' ? 'vk' : 'telegram'; $botMeta = $botType === 'vk' ? (!empty($bot['vk_screen_name']) ? 'vk.com/' . ltrim((string)$bot['vk_screen_name'], '@') : 'VK') : (!empty($bot['bot_username']) ? '@' . (string)$bot['bot_username'] : 'Telegram'); ?>
                    <option value="<?php echo (int)$bot['id']; ?>" <?php echo (int)$bot['id'] === $safeBotId ? 'selected' : ''; ?>><?php echo $h((string)$bot['title'] . ' · ' . ($botType === 'vk' ? 'ВК' : 'TG') . ($botMeta !== '' ? ' · ' . $botMeta : '')); ?></option>
                <?php endforeach; ?>
            </select>
    ...
    <div class="tg-subs-emergency-list">
        <?php foreach ($safeRows as $row):
            $channelType = strtolower((string)($row['channel_type'] ?? $row['platform'] ?? 'telegram')) === 'vk' ? 'vk' : 'telegram';
            $channelShort = $channelType === 'vk' ? 'VK' : 'TG';
            ...
            if ($name === '' && $channelType === 'telegram' && !empty($row['username'])) $name = '@' . ltrim((string)$row['username'], '@');
            if ($name === '') $name = 'Подписчик #' . (int)$row['id'];
            $personMeta = '';
            if ($channelType === 'vk') {
                $personMeta = 'VK User ID ' . (string)($row['external_user_id'] ?? $row['telegram_user_id'] ?? '');
                if (!empty($row['external_chat_id'])) $personMeta .= ' · Peer ID ' . (string)$row['external_chat_id'];
            } else {
                $personMeta = (!empty($row['username']) ? '@' . ltrim((string)$row['username'], '@') . ' · ' : '') . 'Telegram User ID ' . (string)($row['telegram_user_id'] ?? '');
            }
            $channelMeta = '';
            if ($channelType === 'vk') {
                $channelMeta = !empty($row['vk_screen_name']) ? 'vk.com/' . ltrim((string)$row['vk_screen_name'], '@') : 'VK-канал';
            } else {
                $channelMeta = !empty($row['bot_username']) ? '@' . (string)$row['bot_username'] : 'Telegram-канал';
            }
            ...
            <div class="tg-subs-emergency-row" data-subscriber-row="<?php echo (int)$row['id']; ?>">
                ...
                <div class="tg-subs-emergency-avatar"><img src="<?php echo $h($channelIcon); ?>" alt="" aria-hidden="true"></div>
                <div><a class="tg-subs-emergency-name tg-subs-emergency-open" href="<?php echo $h($openUrl); ?>"><?php echo $h($name); ?></a><span class="tg-subs-platform-badge <?php echo $channelType === 'vk' ? 'is-vk' : 'is-telegram'; ?>"><?php echo $h($channelShort); ?></span><div class="tg-subs-emergency-meta"><?php echo $h($personMeta); ?></div><?php $rowTags = $safeRowsTags[(int)$row['id']] ?? []; if ($rowTags): ?><div class="tg-subs-emergency-tags"><?php foreach (array_slice($rowTags, 0, 4) as $tag): ?><span class="tg-subs-emergency-tag"><?php echo $h((string)$tag['name']); ?></span><?php endforeach; ?><?php if (count($rowTags) > 4): ?><span class="tg-subs-emergency-tag">+<?php echo count($rowTags) - 4; ?></span><?php endif; ?></div><?php endif; ?></div>
                <div><div class="tg-subs-emergency-name"><?php echo $h((string)($row['bot_title'] ?? 'Канал')); ?><span class="tg-subs-platform-badge <?php echo $channelType === 'vk' ? 'is-vk' : 'is-telegram'; ?>"><?php echo $h($channelLabel); ?></span></div><div class="tg-subs-emergency-meta"><?php echo $h($channelMeta); ?></div></div>
