...
require_once dirname(__DIR__) . '/scenario_state_service.php';
require_once dirname(__DIR__) . '/scenario_journal_service.php';
    ...
    <section class="tg-contact-page">
        <div class="tg-contact-head">
            <div class="tg-contact-person">
                ...
                <div class="min-w-0"><div class="tg-contact-name"><?php echo $h($displayName); ?><span class="tg-platform-badge <?php echo $channelType === 'vk' ? 'is-vk' : 'is-telegram'; ?>"><?php echo $channelLabel; ?></span></div><div class="tg-contact-sub"><?php echo $h((string)($subscriber['bot_title'] ?? 'Канал')); ?><?php if ($channelType === 'vk' && !empty($subscriber['vk_screen_name'])): ?> · vk.com/<?php echo $h(ltrim((string)$subscriber['vk_screen_name'], '@')); ?><?php elseif ($channelType === 'telegram' && !empty($subscriber['username'])): ?> · @<?php echo $h(ltrim((string)$subscriber['username'], '@')); ?><?php endif; ?></div></div>
            </div>
        ...
        <div class="tg-contact-shell">
            <aside class="tg-contact-side">
                ...
                <?php asr_tg_render_subscriber_scenario_state_card($pdo, (int)$subscriberId, (int)$subscriber['bot_id'], $h, asr_tg_can('flows')); ?>
                <?php asr_tg_render_subscriber_scenario_journal($pdo, (int)$subscriberId, (int)$subscriber['bot_id'], $h, 40); ?>
            </aside>
            <main class="tg-contact-chat" id="chat">
                ...
                <form method="POST" class="tg-chat-compose" enctype="multipart/form-data" data-chat-compose>
                    ...
                    <?php $canSendText = in_array($channelType, ['telegram', 'vk'], true); $canSendFile = $channelType === 'telegram'; ?>
                    <textarea name="message_text" placeholder="Введите сообщение..." data-chat-text <?php echo !$canSendText ? 'disabled' : ''; ?>></textarea>
                    <input type="file" name="chat_attachment" class="hidden" data-chat-file accept=".jpg,.jpeg,.png,.webp,.gif,.mp4,.mov,.m4v,.webm,.mp3,.m4a,.ogg,.wav,.pdf,.zip,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt" <?php echo !$canSendFile ? 'disabled' : ''; ?>>
                    <div class="tg-chat-actions"><div class="tg-chat-tool-row"><div class="tg-chat-tools"><button type="button" class="tg-chat-tool tg-chat-tool--icon" data-chat-emoji title="Эмодзи" aria-label="Эмодзи" <?php echo !$canSendText ? 'disabled' : ''; ?>><span class="tg-chat-tool-glyph" aria-hidden="true">☺</span></button><div class="tg-chat-emoji-menu" data-chat-emoji-menu></div></div><div class="tg-chat-tools"><button type="button" class="tg-chat-tool tg-chat-tool--icon" data-chat-macro title="Переменные" aria-label="Переменные" <?php echo !$canSendText ? 'disabled' : ''; ?>><span class="tg-chat-tool-glyph" aria-hidden="true">{}</span></button><div class="tg-chat-macro-menu" data-chat-macro-menu></div></div><button type="button" class="tg-chat-tool tg-chat-tool--icon" data-chat-attach title="Прикрепить файл" aria-label="Прикрепить файл" <?php echo !$canSendFile ? 'disabled' : ''; ?>><span class="tg-chat-tool-glyph" aria-hidden="true">📎</span></button><span class="tg-chat-file-name" data-chat-file-name></span></div><button class="tg-chat-send" <?php echo !$canSendText ? 'disabled' : ''; ?>>Отправить</button></div>
                    <div class="tg-readonly-note"><?php echo $channelType === 'telegram' ? 'Можно отправить текст, эмодзи и один файл до 45 МБ.' : ($channelType === 'vk' ? 'Для VK сейчас доступна текстовая отправка. Файлы подключим отдельным шагом.' : 'Отправка для этого типа канала пока недоступна.'); ?></div>
                </form>
            </main>
        </div>
    </section>
        ...
        <div class="tg-dialog-grid">
                ...
                ?>
                    ...
                    <a class="tg-dialog-item <?php echo $active ? 'is-active' : ''; ?> <?php echo $unreadCount > 0 ? 'is-unread' : ''; ?>" href="<?php echo $h($url); ?>" data-dialog-subscriber-id="<?php echo (int)$sid; ?>" data-dialog-bot-id="<?php echo (int)($row['bot_id'] ?? $selectedBotId); ?>" onclick="return window.asrTgDialogsOpenFromLink ? window.asrTgDialogsOpenFromLink(this,event) : true;">
                        ...
                        <div class="tg-dialog-item-body">
                            ...
                            <div class="tg-dialog-channel"><span class="tg-dialog-platform-badge <?php echo strtolower((string)($row['channel_type'] ?? 'telegram')) === 'vk' ? 'is-vk' : 'is-telegram'; ?>"><?php echo strtolower((string)($row['channel_type'] ?? 'telegram')) === 'vk' ? 'VK' : 'TG'; ?></span> <?php echo $h((string)($row['bot_title'] ?? 'Канал')); ?><?php echo strtolower((string)($row['channel_type'] ?? 'telegram')) !== 'vk' && !empty($row['username']) ? ' · @' . $h($row['username']) : ''; ?> · <?php echo $h($dialogStatusLabels[$dialogStatus] ?? $dialogStatus); ?><?php if ($unreadCount > 0): ?> · <span class="tg-dialog-unread-badge"><?php echo $unreadCount; ?></span><?php endif; ?></div>
            ...
            <main class="tg-dialog-main">
                ...
                <?php else: ?>
                    ...
                    <div class="tg-dialog-chat-head">
                        ...
                        <div class="tg-dialog-chat-person">
                            ...
                            <div class="tg-dialog-person-lines">
                                ...
                                <div class="tg-dialog-person-bot"><span class="tg-dialog-platform-badge <?php echo strtolower((string)($dialogSubscriber['channel_type'] ?? 'telegram')) === 'vk' ? 'is-vk' : 'is-telegram'; ?>"><?php echo strtolower((string)($dialogSubscriber['channel_type'] ?? 'telegram')) === 'vk' ? 'VK' : 'TG'; ?></span> <?php echo $h((string)($dialogSubscriber['bot_title'] ?? 'Канал')); ?></div>
                                <?php if (strtolower((string)($dialogSubscriber['channel_type'] ?? 'telegram')) !== 'vk' && !empty($dialogSubscriber['username'])): ?><div class="tg-dialog-person-user">@<?php echo $h($dialogSubscriber['username']); ?></div><?php endif; ?>
                            </div>
                        </div>
                    ...
                    <?php $dialogChannelType = strtolower((string)($dialogSubscriber['channel_type'] ?? 'telegram')); $dialogCanSendText = in_array($dialogChannelType, ['telegram', 'vk'], true); $dialogCanSendFile = $dialogChannelType === 'telegram'; ?>
                    <?php if (!$dialogCanSendText): ?>
                        <div class="tg-dialog-compose"><div class="tg-dialog-empty">Отправка для этого типа канала пока недоступна.</div></div>
                    <?php else: ?>
                        <form method="POST" class="tg-dialog-compose" enctype="multipart/form-data" data-dialog-compose>
                            ...
                            <input type="file" name="chat_attachment" class="hidden" data-dialog-chat-file accept=".jpg,.jpeg,.png,.webp,.gif,.mp4,.mov,.m4v,.webm,.mp3,.m4a,.ogg,.wav,.pdf,.zip,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt" <?php echo !$dialogCanSendFile ? 'disabled' : ''; ?>>
                            <div class="tg-chat-actions"><div class="tg-chat-tool-row"><div class="tg-chat-tools"><button type="button" class="tg-chat-tool tg-chat-tool--icon" data-dialog-chat-emoji title="Эмодзи" aria-label="Эмодзи"><span class="tg-chat-tool-glyph" aria-hidden="true">☺</span></button><div class="tg-chat-emoji-menu" data-dialog-chat-emoji-menu></div></div><div class="tg-chat-tools"><button type="button" class="tg-chat-tool tg-chat-tool--icon" data-dialog-chat-macro title="Переменные" aria-label="Переменные"><span class="tg-chat-tool-glyph" aria-hidden="true">{}</span></button><div class="tg-chat-macro-menu" data-dialog-chat-macro-menu></div></div><button type="button" class="tg-chat-tool tg-chat-tool--icon" data-dialog-chat-attach title="Прикрепить файл" aria-label="Прикрепить файл" <?php echo !$dialogCanSendFile ? 'disabled' : ''; ?>><span class="tg-chat-tool-glyph" aria-hidden="true">📎</span></button><span class="tg-chat-file-name" data-dialog-chat-file-name></span></div><button class="tg-chat-send" type="submit">Отправить</button></div>
                            <div class="tg-readonly-note"><?php echo $dialogChannelType === 'telegram' ? 'Можно отправить текст, эмодзи и один файл до 45 МБ.' : 'Для VK сейчас доступна текстовая отправка. Файлы подключим отдельным шагом.'; ?></div>
                        </form>
                    <?php endif; ?>
                <?php endif; ?>
            </main>
                ...
                <?php else: ?>
                    ...
                    <div class="tg-dialog-side-section">
                        ...
                        <div class="tg-dialog-detail-block">
                            ...
                            <?php $dialogStaticChannelType = strtolower((string)($dialogSubscriber['channel_type'] ?? 'telegram')) === 'vk' ? 'vk' : 'telegram'; ?>
                            <div class="tg-dialog-info-row"><span>Платформа</span><span><?php echo $dialogStaticChannelType === 'vk' ? 'ВК' : 'Telegram'; ?></span></div>
                            <?php if ($dialogStaticChannelType === 'vk'): ?>
                                <div class="tg-dialog-info-row"><span>VK User ID</span><span><?php echo $h((string)($dialogSubscriber['external_user_id'] ?? $dialogSubscriber['telegram_user_id'] ?? '')); ?></span></div>
                                <div class="tg-dialog-info-row"><span>VK Peer ID</span><span><?php echo $h((string)($dialogSubscriber['external_chat_id'] ?? $dialogSubscriber['chat_id'] ?? '')); ?></span></div>
                                <div class="tg-dialog-info-row"><span>VK адрес</span><span><?php echo !empty($dialogSubscriber['vk_screen_name']) ? 'vk.com/' . $h(ltrim((string)$dialogSubscriber['vk_screen_name'], '@')) : 'не указан'; ?></span></div>
                            <?php else: ?>
                                <div class="tg-dialog-info-row"><span>Telegram User ID</span><span><?php echo $h((string)($dialogSubscriber['telegram_user_id'] ?? '')); ?></span></div>
                                <div class="tg-dialog-info-row"><span>Chat ID</span><span><?php echo $h((string)($dialogSubscriber['chat_id'] ?? '')); ?></span></div>
    ...
    (function(){
        ...
        function renderDialogList(payload){
            ...
            dialogs.forEach(function(row){
                ...
                var rowIsVk=String(row.channel_type||'telegram').toLowerCase()==='vk';html+='<div class="tg-dialog-preview">'+esc(preview)+'</div><div class="tg-dialog-channel"><span class="tg-dialog-platform-badge '+(rowIsVk?'is-vk':'is-telegram')+'">'+(rowIsVk?'VK':'TG')+'</span> '+esc(row.bot_title||row.channel_label||'Канал')+' · '+esc(labels.status[row.status]||row.status||'')+(unread>0?' · <span class="tg-dialog-unread-badge">'+unread+'</span>':'')+'</div>';
        ...
        function renderMainShell(subscriber,row,panelPayload){
            ...
            var html='<div class="tg-dialog-chat-head"><div class="tg-dialog-chat-person"><div class="tg-dialog-avatar"><img src="'+esc(channelIcon(subscriber))+'" alt="" aria-hidden="true"></div><div class="tg-dialog-person-lines"><div class="tg-dialog-person-name">'+esc(subscriber.display_name||'Подписчик')+'<span class="tg-dialog-status-dot2 '+(needs?'is-red':'')+'" title="'+(needs?'Ждёт ответа':'Ответ дан')+'"></span></div><div class="tg-dialog-person-bot"><span class="tg-dialog-platform-badge '+(isVk?'is-vk':'is-telegram')+'">'+(isVk?'VK':'TG')+'</span> '+esc(subscriber.bot_title||'Канал')+'</div>'+(!isVk&&subscriber.username?'<div class="tg-dialog-person-user">@'+esc(subscriber.username)+'</div>':'')+'</div></div>';
            ...
            if(isVk){
                html+='<form method="POST" class="tg-dialog-compose" enctype="multipart/form-data" data-dialog-compose><input type="hidden" name="action" value="tg_subscriber_send_message"><input type="hidden" name="return_page" value="messages"><input type="hidden" name="dialog_view" value="'+esc(dialogState.view)+'"><input type="hidden" name="bot_id" value="'+esc(subscriber.bot_id)+'"><input type="hidden" name="subscriber_id" value="'+esc(subscriber.id)+'">'+csrfInput()+'<textarea name="message_text" placeholder="Введите сообщение..." data-dialog-chat-text></textarea><input type="file" name="chat_attachment" class="hidden" data-dialog-chat-file disabled><div class="tg-chat-actions"><div class="tg-chat-tool-row"><div class="tg-chat-tools"><button type="button" class="tg-chat-tool tg-chat-tool--icon" data-dialog-chat-emoji title="Эмодзи" aria-label="Эмодзи"><span class="tg-chat-tool-glyph" aria-hidden="true">☺</span></button><div class="tg-chat-emoji-menu" data-dialog-chat-emoji-menu></div></div><div class="tg-chat-tools"><button type="button" class="tg-chat-tool tg-chat-tool--icon" data-dialog-chat-macro title="Переменные" aria-label="Переменные"><span class="tg-chat-tool-glyph" aria-hidden="true">{}</span></button><div class="tg-chat-macro-menu" data-dialog-chat-macro-menu></div></div><button type="button" class="tg-chat-tool tg-chat-tool--icon" data-dialog-chat-attach title="Файлы для VK подключим отдельным шагом" aria-label="Прикрепить файл" disabled><span class="tg-chat-tool-glyph" aria-hidden="true">📎</span></button><span class="tg-chat-file-name" data-dialog-chat-file-name></span></div><button class="tg-chat-send" type="submit">Отправить</button></div><div class="tg-readonly-note">Для VK сейчас доступна текстовая отправка. Файлы подключим отдельным шагом.</div></form>';
            }
        ...
        function renderSide(panelPayload,row){
            ...
            var sIsVk=String(s.channel_type||'telegram').toLowerCase()==='vk';
            ...
            html+=infoRow('Платформа',sIsVk?'ВК':'Telegram');
            if(sIsVk){
                html+=infoRow('VK User ID',s.external_user_id||s.telegram_user_id||'');
                html+=infoRow('VK Peer ID',s.external_chat_id||s.chat_id||'');
                html+=infoRow('VK адрес',s.vk_screen_name?('vk.com/'+String(s.vk_screen_name).replace(/^@/,'')):'не указан');
            } else {
                html+=infoRow('Telegram User ID',s.telegram_user_id||'');
